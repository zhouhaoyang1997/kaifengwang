...
function showNewPage(tit, html, callback) {
    var windowIframe = $('#windowIframe'), windowIframeTitle = $('#windowIframeTitle'),
        windowIframeBody = $('#windowIframeBody');
...
function windowToShow(action_name, to_url, ok_url) {
    var html = '<s class="s s1"></s><s class="s s2"></s>' + '<div class="txt">请在菜单中选择在浏览器中打开，<br>以完成' + action_name + '。</div>' + '<a href="' + ok_url + '" class="btn">已完成' + action_name + '</a>　<a href="' + ok_url + '" class="btn">取消并返回</a>';
...
function MympsWindowMsg(action, showid, str, url, formcode) {
    ...
    var sys_tips = $('#sys_tips'), sys_tips_title = $('#sys_tips_title'), sys_tips_info = $('#sys_tips_info'),
        sys_tips_submit = $('#sys_tips_submit');
    if (showid == "0") {
...
function reloadLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(showMapGPSre, handleError, {
...
function getLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(showMapGPS, handleError, {
            enableHighAccuracy: true,
            maximumAge: 1000,
            timeout: 6000
        });
...
function showFilter(option) {
    var node = $('#' + option.ibox), fullbg = $('#' + option.fullbg), ct1 = $('#' + option.content1),
        ct2 = $('#' + option.content2), ctp1 = ct1.find('.innercontent'), ctp2 = ct2.find('.innercontent'),
        currentClass = 'current';
    var tabs = node.find('.tab .item'), conts = node.find('.inner');
